...
def _gen_init_schema(user_input: dict):
    return vol.Schema({
        vol.Required("name", default=user_input.get("name")): cv.string,
        vol.Required("unit", default=user_input.get("unit")): vol.In(unit_types),
        vol.Required("type", default=user_input.get("type")): vol.In(device_types),
        vol.Required("value", default=user_input.get("value")): cv.positive_float,
    })
...
class ConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input):
        ...
        if not user_input:
            user_input = dict(name="My Meter", unit="kWh", value=0, type=None)
